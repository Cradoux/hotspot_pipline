...
class GetHotspots(luigi.Task):
    ...
    def run(self):
        ...
        s.sphere_maps = True
        s.nrotations = 3000
        hr = h.from_protein(prot, buriedness_method='ghecom', nprocesses=1, settings=s, cavities=mol)
...
class SaveTractabilityMap(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget("pdb_files/{}/{}/tractability_single/out.zip".format(self.pdb[1:3], self.pdb))

    def run(self):
        hr = HotspotReader(self.input().path).read()
        bcv = hr.tractability_map(mvon=False)
        out_settings = HotspotWriter.Settings()
        out_settings.charged = False
        w = HotspotWriter(os.path.dirname(self.output().path), grid_extension=".grd", zip_results=True,
                          settings=out_settings)
...
class WriteTractabilityScores(luigi.Task):
    ...
    def requires(self):
        return SaveTractabilityMap(self.pdb)

    def output(self):
        return luigi.LocalTarget("pdb_files/{}/{}/tractability_single/scores.csv".format(self.pdb[1:3], self.pdb))
...
class LotsOTasks(luigi.WrapperTask):
    ...
    def requires(self):
        ...
        for pdb in pdb_li:
            yield WriteTractabilityScores(pdb=pdb)
